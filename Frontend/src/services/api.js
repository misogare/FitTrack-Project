...
deleteWorkout: (id) => request(`/workouts/${id}`, { method: 'DELETE' }),
  ...
  // Account settings (steps / minutes / burn / hydration / sharing toggles / prefs)
  settings: () => request('/settings'),
  updateSettings: (payload) => request('/settings', { method: 'PUT', body: JSON.stringify(payload) }),
  deleteAllData: (payload) => request('/auth/delete-data', { method: 'POST', body: JSON.stringify(payload) }),
  deleteAccount: (payload) => request('/auth/account', { method: 'DELETE', body: JSON.stringify(payload) }),
  plans:        () => request('/plans'),
